...
-- For example:
    ...
    for k,v in pairs(obj) do
       --socket.try(c:send(string.format("t = %.2f, ID = %d, name = %s, country = %s(%s), LatLongAlt = (%f, %f, %f), heading = %f\n", t, k, v.Name, v.Country, v.Coalition, v.LatLongAlt.x, v.LatLongAlt.Long, v.LatLongAlt.Alt, v.Heading)))
		...
		if k == ID
		then
		msg = string.format("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f;",t, vv.x, vv.y, vv.z, MAGyaw, altBar, altRad,VerticalVelocity,pitch,bank,yaw,engine.RPM.left,engine.RPM.right,CameraPosition.x.x,CameraPosition.y.x,CameraPosition.z.x)
		mssage = table.concat({mssage,msg})
		elseif p1 == 28
		then
		msg = string.format("%f,%f,%f,%f,%f,%f,%f",t, p5.x, p5.y, p5.z, p6, p7, p8)
        --msg = string.format("%f,%f,%f,%f,%f,%f,%f",HSI.HeadingPointer, HSI.BearingPointer, p5.y, p5.z, p6, p7, p8)
        ...
        --rou=string.format("%f,%f,%f;",Route.goto_point.world_point.x,Route.goto_point.world_point.y,Route.goto_point.world_point.z)
       -- route_mess = table.concat({route_mess,rou})
    ...
    socket.try(c:send(string.format("ro%s",route_mess)))
