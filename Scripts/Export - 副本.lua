...
-- 2) Socket
  ...
  socket = require("socket")
	...
	e = socket.udp()
    e:setpeername(host, port1)
    e:settimeout(.01) -- set the timeout for reading the socket
	-- receive
	d = socket.udp()
    d:setsockname('*', port2)
...
-- 1) File
 -- if default_output_file then
   ...
   -- end
-- 2) Socket
	--socket.try(c:send(string.format("%.2f,IAS)))
	
	--socket.try(c:send(string.format("t = %.2f, name = %s, Long = %.2f, Lat = %.2f, Alt = %.2f\n",t,name, Long, Lat, Alt)))
    socket.try(c:send(string.format("t = %.2f, name = %s, altBar = %.2f, alrRad = %.2f, pitch = %.2f, bank = %.2f, yaw = %.2f", t, name, altRad, altBar, pitch, bank, yaw)))
	--socket.try(c:send(altRad))
	--socket.try(c:send(IAS))
	--ProcessInput()
...
function ProcessInput()
	...
	data = d:receive()	
	--socket.try(c:send(data))
	data_head = string.sub(data,1,3)
	data_cent = string.sub(data,4)
--[[
		if data_head=="pch" then
			num = tonumber(data_cent)
			num = (num-500)/1000
			LoSetCommand(2001,num)
		end
		if data_head=="rll" then
			num = tonumber(data_cent)
			num = (num-500)/1000
			LoSetCommand(2002,num)
		end 
		if data_head=="nll" then
			num = tonumber(data_cent)
			num = (num-500)/500
			LoSetCommand(2087,num)
		end 
--]]
		if data_head=="neu" then
		    
			LoSetCommand(2004,data_cent)
			  if data_cent==1 then
			  LoSetCommand(73,1)
			  end
		end 
